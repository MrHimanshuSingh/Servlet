...
@WebFilter("/userAddPro")
@MultipartConfig
public class UserAddProductFilter implements Filter {
	@Override
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (emSubject && emEdition && emPrice && emUniversity && emStream && emBranch && emSemester && emPublication && emImg1
				&& emImg2 && emImg3) {
			chain.doFilter(request, response);
		} else {
			req.getRequestDispatcher("sell.jsp").forward(request, response);
		}
	}
